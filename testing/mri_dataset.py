...
from fastmri_class import INDEX2_helper
...
class RealMeasurement(Dataset):
    def __init__(
        ...
        # self.mask1, self.mask2 = _mask_fn['uniformly_cartesian']((512, 256), acceleration_rate)
    ...
    def __getitem__(self, item):
        ...
        if self.is_return_y_smps_hat:
            ...
            #print(f"size of x_hat is {x_hat.size()}")

            x_hat = torch.from_numpy(x_hat)
            x_hat = torch.view_as_real(x_hat)
            x_hat = x_hat.moveaxis(-1, 0)
            x_hat = (x_hat - x_hat.min()) / (x_hat.max() - x_hat.min())

            #x_hat = x_hat.abs()
            #x_hat = torch.stack([x_hat, torch.zeros(x_hat.size())])
